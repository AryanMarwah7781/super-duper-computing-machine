...
interface LanyardProps {
  ...
  lanyardWidth?: number;
  /** Length of each of the three rope segments. Lower hangs the badge higher. */
...
function Band({
  ...
  return (
    <>
      <group position={[0, 1 + segmentLength * 3, 0]}>
      ...
      <mesh ref={band}>
        <meshLineGeometry />
        <meshLineMaterial
          // R3F requires constructor args on extended materials. The prop of
          // the same name below is what actually drives it each render; this
          // only satisfies the constructor's required field.
          args={[{ resolution: new THREE.Vector2(1000, 1000) }]}
          color="white"
          depthTest={false}
          resolution={isMobile ? [1000, 2000] : [1000, 1000]}
          // meshline types useMap as a float, not a boolean; the bare `useMap`
          // shorthand from the registry fails typecheck.
          useMap={1}
          map={texture}
          repeat={[-4, 1]}
